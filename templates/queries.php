...
function selectProductsQty()
{
	...
	$query = $wpdb->prepare("SELECT c.last_name, c.first_name, pr.order_item_name, SUM(p.product_qty) as 'sum_product_qty'
	...
	foreach($result as $row)
	{
		echo '<tr>' .
			...
			'<td style="text-align: center;">' . $row['sum_product_qty'] . '</td>' .
			'</tr>';
	}
	echo '</table>';
}
...
function selectAvgofOrders()
{
	...
	$query = $wpdb->prepare("SELECT c.last_name, c.first_name , round(AVG(ord.total_sales), 2) as 'avg_total_sales'
	FROM {$wpdb->prefix}wc_customer_lookup as c 
	INNER JOIN  {$wpdb->prefix}wc_order_stats as ord
	ON ord.customer_id = c.customer_id
	WHERE c.email = %s
...
function selectLatestOrder(){
	...
	$query = $wpdb->prepare("SELECT c.last_name, c.first_name, MAX(ord.date_created) as 'max_date'
	FROM {$wpdb->prefix}wc_customer_lookup as c 
	INNER JOIN  {$wpdb->prefix}wc_order_stats as ord
...
{
	global $wpdb;
	$query = $wpdb->prepare("SELECT ord.order_id, c.last_name, c.first_name, ord.date_created
	FROM {$wpdb->prefix}wc_customer_lookup as c 
	INNER JOIN  {$wpdb->prefix}wc_order_stats as ord
	ON ord.customer_id = c.customer_id
	WHERE c.email = %s 
